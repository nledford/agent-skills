...
| `internationalization-localization` | Internationalization, localization, Project Fluent, Fluent Translation List (`.ftl`) authoring, locale fallback, multilingual catalogs, localized formatting, and cross-stack Fluent integration guidance. | Add. Fills durable i18n/l10n coverage without duplicating implementation mechanics owned by language, API, styling, security, testing, and documentation skills. |
| `javascript-typescript-engineering` | JS/TS source, TypeScript, package managers, scripts, tests, lint/format/build, dependencies, workspaces, and CLIs. | Rename from `bun-javascript-workflows`. Covers JS/TS broadly with Node/npm defaults while preserving Bun guidance when local evidence requires it. |
...
| `serena` | MCP-based semantic code navigation, diagnostics, symbol/reference discovery, and symbol-level edits/renames for supported languages. | Add. Provides first-party guidance for IDE-like code tooling while preserving direct file reads, repository docs/instructions, and validation commands as source-of-truth checks. |
