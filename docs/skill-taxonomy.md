...
| Public web research | `hound-web-research` | Public source discovery, page and PDF retrieval, bounded crawling, OCR, visual evidence, source verification, and safe Hound MCP operation; repository-local evidence, authenticated interaction, and durable browser tests remain elsewhere. |
| Project workflow tools | `git-commit`, `git-workflows`, `justfiles`, `suggest-lucide-icons` | Narrow operational guidance for constructing commits, repository-state Git workflows, Justfiles, and verified Lucide icon names. |
...
| Public web research and source retrieval | `hound-web-research`, `security-review`, `security-review-evidence`, `agent-browser`, `playwright-e2e` | Baseline complete | Covers sanitized public search, authoritative-source retrieval, known URLs and PDFs, bounded same-domain crawling, OCR and screenshots, prompt-injection resistance, cache/version diagnostics, authenticated-browser and durable-test boundaries, and security handoffs. |
