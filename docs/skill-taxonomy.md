...
| Data, identifiers, and SQL | `data-platform-engineering`, `random-data-identifiers`, `sql-engineering`, `postgresql-sql-engineering`, `sqlite-sql-engineering` | Analytical data-platform ingestion, transformations, semantics, Power BI models, and operations; randomness/IDs; database-neutral SQL; and engine-specific PostgreSQL/SQLite schema, migration, query, transaction, performance, security, and review guidance. |
...
| `data-platform-engineering` | Source-to-landing ingestion, post-landing analytical transformations, cross-cutting analytical semantics, Power BI semantic models, and Fabric or Power BI platform operations, with progressive lane-specific references. | Application domain modeling, physical OLTP design, generic distributed protocols, generic UX, and final release decisions remain with their owning skills. |
...
| `data-platform-engineering` | Analytical data-platform ingestion, lakehouse or warehouse transformations, analytical semantic contracts, Power BI semantic models and DAX, and Fabric or Power BI operations. | Uses five progressively loaded references and keeps application DDD, physical database design, generic concurrency, generic UX, and final release decisions with their existing owners. |
...
| `prompt-engineering-review` | Review and, when requested, rewrite coding-agent prompts, roles, delegation instructions, bounded multi-agent handoff contracts, acceptance criteria, and quality gates. | Owns prompt behavior and agent-instruction interface clarity while application architecture, runtime coordination, security controls, test strategy, release readiness, and reusable skill contracts remain with their focused workflows. |
...
| Analytical data platforms | `data-platform-engineering`, SQL, API, observability, performance, testing, documentation, security, and release-readiness skills | Baseline complete | Covers source-to-landing ingestion, post-landing transformations and technical lineage, cross-cutting analytical semantics and business lineage, Power BI models and DAX, and Fabric or Power BI operations through five progressive references. |
...
```gherkin
Scenario: Analytical data-platform work loads only material responsibility lanes
  Given a change touches ingestion, post-landing transformations, analytical semantics, Power BI semantic models, or Fabric and Power BI operations
  When the agent chooses data-platform guidance
  Then it loads data-platform-engineering and only the references for materially affected responsibility lanes
  And it may load analytical-semantics guidance alongside transformation or Power BI guidance when governed meaning changes
  And it distinguishes technical execution lineage from semantic and business lineage
  And application DDD, physical database design, generic distributed protocols, generic UX, and the final release decision remain with their owning skills
```
